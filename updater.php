...
<html lang="en">
  ...
  <body>
  	<div class="container">
  		...
  		<div class="right">
				<h1>Upgrading Premium URL Shortener to 4.2</h1> 
				<p>
					You are about to upgrade this software to version <strong>4.2</strong>. Please note that this will only update your database and NOT your files. It is strongly recommended that you first backup your database then your existing files in case something unexpected occurs. 
				</p>
				<p>
					Version 4 adds many new functionality including improvements in performance, features and security. For this reason, <strong>a lot of files</strong> were updated. <strong>Please read</strong> the update manual carefully in order to make sure the update is done as smoothly as possible. 
				</p>			
				<p>					
...
function get_query($dbinfo){
	...
	//Update stats Table
	$query[]="ALTER TABLE `{$dbinfo["prefix"]}stats` ADD `urlid` bigint(20) NOT NULL";
	$query[]="ALTER TABLE `{$dbinfo["prefix"]}stats` ADD `urluserid` bigint(20) NOT NULL DEFAULT '0'";
	$query[]="ALTER TABLE `{$dbinfo["prefix"]}stats` ADD `domain` varchar(50) NOT NULL";
 	// Update URL Table
 	$query[]="ALTER TABLE `{$dbinfo["prefix"]}url` ADD `type` varchar(50) NOT NULL DEFAULT ''";
 	// Update User Table
 	$query[]="ALTER TABLE `{$dbinfo["prefix"]}user` ADD `auth_key` varchar(100) NOT NULL";
 	$query[]="ALTER TABLE `{$dbinfo["prefix"]}user` ADD `last_payment` datetime NOT NULL";
 	$query[]="ALTER TABLE `{$dbinfo["prefix"]}user` ADD `expiration` datetime NOT NULL";
 	$query[]="ALTER TABLE `{$dbinfo["prefix"]}user` ADD `pro` int(1) NOT NULL DEFAULT '0'";
